...
# --- DynamoDB Table for Typing Texts ---

resource "aws_dynamodb_table" "typing_texts" {
  name           = "TypingTexts" # Name for the new table
  billing_mode   = "PAY_PER_REQUEST"
  hash_key       = "Theme"       # Partition Key: Category of the text
  range_key      = "TextID"      # Sort Key: Unique ID within the theme

  # --- Attribute Definitions ---
  attribute {
    name = "Theme" # e.g., "formal", "novel", "scientific"
    type = "S"     # String
  }
  attribute {
    name = "TextID" # e.g., "formal-001", "novel-austen-01", "tech-react-hooks-05"
    type = "S"      # String
  }
  # Note: Other attributes like ParagraphText, Difficulty, Source, Length
  # do not need to be defined here as they are not part of the key schema.

  tags = {
    Name        = "TypingTexts"
    Environment = "Production" # Or your desired environment
    Purpose     = "Content for Typing Game"
  }
}

# --- Output for the new table ---

output "typing_texts_table_name" {
  value       = aws_dynamodb_table.typing_texts.name
  description = "The name of the DynamoDB table for typing texts"
}

output "typing_texts_table_arn" {
  value       = aws_dynamodb_table.typing_texts.arn
  description = "The ARN of the DynamoDB table for typing texts"
}
